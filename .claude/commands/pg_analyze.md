---
description: 遗留系统梳理命令 - 在老旧项目中，梳理现有架构与技术债，生成「现状程序设计文档」作为后续 /pg_plan 的输入基础 (Legacy System Analysis Command)
---

当用户输入 `/pg_analyze` 命令时，你必须严格按照以下步骤执行：
...
> 分析时间：[当前日期]
> 分析范围：[描述了哪些材料/文件]
> 文档用途：作为新需求开发时的「已有系统参考」，供 /pg_plan 使用
...
1. **探测真实项目根目录**（多模块项目关键步骤）：

   用户提供的路径可能是某个子模块路径（如 `trade-cart/trade-cart-service/`），此时必须向上寻找真正的项目根，**不能直接把 module 路径当作项目根**。

   探测规则（按优先级，找到即停止）：
   - **向上逐级查找** 含 `.git` 目录的路径 → 该路径即为项目根（Git 仓库根）
   - **向上逐级查找** 含顶层 `pom.xml`（即 `<packaging>pom</packaging>` 或不含 parent 的 pom）的路径 → 该路径即为多模块 Maven 根
   - 若两者都找不到，以用户提供的路径本身作为项目根

   > 示例：用户给出 `/project/trade-cart/trade-cart-service/`，向上发现 `/project/trade-cart/` 有 `.git`，
   > 则项目根 = `/project/trade-cart/`，文档写到 `/project/trade-cart/pg/doc/`

2. **确定存储目录**（按优先级）：
   - 已有 `spec-status.json`（在项目根或 module 内） → 使用其中的 `specDir`
   - 探测到项目根 → `[项目根]/pg/doc/`
   - 否则 → 询问用户指定目录

3. **立即写入文件**，写完后在回复中仅输出**摘要**，不重复文档内容：
...
⏭ **建议下一步**：
1. `/pg_spec [需求名称]` 初始化新需求
2. `/pg_prd [需求来源]` 生成需求文档
3. `/pg_plan` 时直接引用本文档作为「已有系统参考」

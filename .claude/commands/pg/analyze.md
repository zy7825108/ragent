...
> ⚡ 输出完毕后**立即写文件**：`现状设计_[系统名称]_YYYYMMDD.md`，不等用户确认。

严格按以下格式输出，无内容章节写「暂无」，不得省略：
...
2. **立即写入文件**：`现状设计_[系统名称]_YYYYMMDD.md`

3. 在回复中输出（Markdown 格式，非代码块）：

---

✅ **现状程序设计文档已生成并保存至：`[文件路径]`**
...
- ❌ 禁止省略技术债章节（即使代码质量很好，也需明确标注「暂无明显技术债」）
- ❌ 禁止美化历史设计（如实反映，帮助用户对包袱心中有数）
- ❌ 禁止只输出内容不写文件（分析完成后必须立即写入 `现状设计_xxx.md`）
